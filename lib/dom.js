...
var arrayNodeListHandler = {
	get: function(target, property) {
		if(property === 'length') {
			return target._array.length;
		}else{
			var index = parseInt(property);
			if(!isNaN(index)) {
				return target._array[index];
			}else{
				console.warn("arrayNodeListHandler.get(): Accessing '" + property + "'");
				return target[property];
			}
		}
	},
	set: function(target, property, value) {
		console.warn("arrayNodeListHandler.set(): Accessing '" + property + "'");
		return target.property = value;
	}
};

function makeArrayNodeList(array) {
	return new Proxy({ _array: array }, arrayNodeListHandler);
}
...
function Element(document, tag_name) {
	...
	// --------------- inherited from Node ---------------
	this.childNodes = makeArrayNodeList(this._realChildren);
	// --------------- defined in Element ---------------
...
Text.prototype.childNodes = makeArrayNodeList([ ]);
...
Document.prototype.createElement = function(tag_name) {
	...
	var element_class = html.elementMap[tag_name];
	if(element_class)
		return new element_class(this);
	return new html.HTMLUnknownElement(this, tag_name);
};
